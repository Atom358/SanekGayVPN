...
Сервис работает по принципу подписки. Вы вставляете [ссылку](https://github.com/Atom358/SanekGayVPN?tab=readme-ov-file#ссылка) в специальное приложение — прокси-клиент, поддерживающий протоколы VPN (например, Hiddify, Happ, NekoBox, v2rayNG или другие). После импорта подписки программа автоматически загрузит список доступных серверов. Чтобы начать пользоваться, выберите любой сервер из списка и нажмите «Подключиться». Трафик будет маршрутизироваться через VPN, сохраняя вашу активность в интернете приватной.
