...
Сервис работает по принципу подписки. Вы вставляете [ссылку](https://github.com/Atom358/SanekGayVPN#ссылка) в специальное приложение — прокси-клиент, поддерживающий протоколы VPN (рекомендую Happ, NekoBox, INCY или Hiddify). После импорта подписки программа автоматически загрузит список доступных серверов. Чтобы начать пользоваться, выберите любой сервер из списка и нажмите «Подключиться». Трафик будет маршрутизироваться через VPN, сохраняя вашу активность в интернете приватной.

Подписка обновляется автоматически каждый час, но вы также можете сделать это вручную через кнопку «Обновить». Перед подключением рекомендуется проверять пинг до сервера — например, через встроенную функцию «URL-тест» (HTTP GET via Proxy[^1]). Если пинг высокий или соединение нестабильно, просто переключитесь на другой сервер из списка.
